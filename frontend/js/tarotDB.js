...
function getCardImagePath(card) {
    // 首先尝试直接访问，因为大多数文件都直接存放在cards目录
    const directPath = `./cards/${card.image}`;
    
    // 检查card.type和card.suit，为后续可能的子目录组织做准备
    if (card.type === 'major') {
        // 返回两种可能的路径，由validateCardPath尝试
        return `./cards/major/${card.image}`;
    } else if (card.suit) {
        // 针对小阿卡纳，返回子目录路径
        return `./cards/${card.suit}/${card.image}`;
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        const tryPath = (currentPath) => {
            ...
            img.onerror = () => {
                ...
                // 如果是子目录路径失败，尝试直接从cards目录加载
                if (currentPath.includes('/major/') || 
                    currentPath.includes('/cups/') || 
                    currentPath.includes('/wands/') || 
                    currentPath.includes('/swords/') || 
                    currentPath.includes('/pentacles/')) {
                    ...
                    // 构建直接的路径
                    const directPath = `./cards/${fileName}`;
                    console.log('尝试备用路径:', directPath);
                    
                    // 尝试备用路径
                    tryPath(directPath);
                } else {
                    // 如果所有尝试都失败，那么拒绝Promise
                    reject(new Error(`图片加载失败: ${currentPath}`));
                }
            };
