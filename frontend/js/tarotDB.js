...
function getCardImagePath(card) {
    ...
    if (card.image) {
        // 可能是上传到顶层目录的cups卡牌
        const cupMatch = card.image.match(/(\d+)-(.+)-of-Cups\.jpg/i);
        if (cupMatch) {
    ...
    // 构建完整的图片路径 (使用相对路径)
    const imagePath = `./cards/${subDir}/${card.image}`;
    
    // 根据您的文件名格式调整
    if (card.suit === 'cups') {
        // 检查牌的编号和类型，例如 "01-Ace-of-Cups.jpg"
        const cardNumber = String(card.id).padStart(2, '0');
        const nameMatch = card.nameEN.match(/^(\w+)-of-Cups$/i);
        if (nameMatch) {
            const cardType = nameMatch[1];
            return `./cards/${cardNumber}-${cardType}-of-Cups.jpg`;
        }
    }
