...
这是一个基于Python的八字命理分析系统，可以根据用户的出生年月日时进行八字排盘和分析。
...
1. 启动API服务器
   ...
   Windows用户：
   双击 `start_server.bat` 文件启动服务器

   Linux/Mac用户：
   ```bash
   python bazi_bridge.py
   ```

2. 在浏览器中访问以下地址：
   
   ```
   file:///[您的路径]/卦/bazi详细分析.html?name=张三&gender=男&year=1990&month=6&day=15&hour=9&minute=30
   ```
   
   请将其中的 `[您的路径]` 替换为实际路径，并根据需要修改URL参数中的出生信息。

## API接口说明

### 1. 八字分析接口

- URL: `http://localhost:5000/api/bazi/analyze`
- 方法: POST
- 参数:
  - name: 姓名
  - gender: 性别（男/女）
...
- URL: `http://localhost:5000/api/bazi/demo`
- 方法: GET
- 无需参数
...
- `ganzhi.py`: 干支数据和计算函数
- `common.py`: 公共函数和工具
- `bazi_bridge.py`: API服务器，连接前端和后端
- `bazi详细分析.html`: 八字详细分析前端页面

## 注意事项

- 本系统仅供研究和学习使用，不宜用于商业目的
- 分析结果仅供参考，请理性看待
- API服务器需要保持运行才能正常使用分析功能
